"""
This script contains additional utility functions used by other scripts. Most functions are from the Hamiltorch library
"""
...
def progress_bar_init(message, num_iters, iter_name='Items', rejections=False):
    ...
    if not rejections:
        ...
        print('Time spent  | Time remain.| Progress             | {} | {}/sec | Rejected Samples'.format(
            iter_name.ljust(progress_bar_len_str_num_iters * 2 + 1), iter_name))
    ...
    if rejections is None:
        if (duration - progress_bar_prev_duration > _print_refresh_rate) or (iter >= progress_bar_num_iters - 1):
            ...
            print('{} | {} | {} | {}/{} | {:,.2f}       '.format(days_hours_mins_secs_str(duration),
                                                                 ...
                                                                 progress_bar_num_iters, traces_per_second), end='\r')
            sys.stdout.flush()
    else:
        if (duration - progress_bar_prev_duration > _print_refresh_rate) or (iter >= progress_bar_num_iters - 1):
            progress_bar_prev_duration = duration
            ...
            print('{} | {} | {} | {}/{} | {:,.2f} |  {:,.2f}     '.format(days_hours_mins_secs_str(duration),
                                                                          ...
                                                                          progress_bar_num_iters, traces_per_second,
                                                                          rejections), end='\r')
            sys.stdout.flush()
...
_internal_attrs = {'_backend', '_parameters', '_buffers', '_backward_hooks', '_forward_hooks', '_forward_pre_hooks',
                   '_modules'}
...
def make_functional(module):
    ...
    def fmodule(*args, **kwargs):
        params_box[0] = kwargs.pop(
            'params')  # if key is in the dictionary, remove it and return its value, else return default. If default is not given and key is not in the dictionary, a KeyError is raised.
...
###### Utils for DeepONets ###################
def load_samps(file_name):
    samples = np.load(file_name, allow_pickle=True)
    samp_list = []
    for i in range(samples.shape[0]):
        samp_list.append(torch.tensor(samples[i]))
...
def get_cone_data():
    raise NotImplementedError('Cone dataset is not available')


def get_burgers_data(cfg):
